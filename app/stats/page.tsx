...
export default function StatsPage() {
  const { actions, blazes, sparks, branches, pathways, goals } = useStore();
  ...
  const scheduleGroups: Array<{ label: 'Today' | 'Tomorrow' | 'This Week'; list: typeof goals }> = [
    { label: 'Today', list: todayScheduled },
    { label: 'Tomorrow', list: tomorrowScheduled },
    { label: 'This Week', list: thisWeekScheduled },
  ];
  ...
  return <Layout><h2 className="mb-4 text-2xl font-semibold tracking-tight">Progress</h2><section className='mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4'>{topCards.map(([label,val])=><article key={label} className={`rounded-xl border border-neon/20 bg-panelAlt/80 p-3 ${motionStyles.selectable}`}><p className='text-xs uppercase tracking-widest text-neonDim'>{label}</p><p className='mt-1 text-2xl font-semibold text-amber-100'>{val}</p></article>)}</section>
    ...
    <section className={`mb-3 ${sectionStyles.archive}`}><details><summary className='cursor-pointer text-sm uppercase tracking-widest text-neonDim'>History</summary><ul className="mt-3 space-y-1 text-sm text-muted">{actions.map((a)=><li key={a.id}>{a.date} • {a.action}</li>)}</ul></details></section>
  </Layout>;
}

function Section({ title, children, variant = 'analytics' }: { title: string; children: React.ReactNode; variant?: keyof typeof sectionStyles }) { return <section className={`mb-3 ${sectionStyles[variant]}`}><h3 className="text-sm uppercase tracking-widest text-neonDim">{title}</h3><div className="mt-3 space-y-2">{children}</div></section>; }
