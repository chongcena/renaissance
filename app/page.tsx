...
export default function HomePage() {
  ...
  const recentBlazes = [...blazes].sort((a, b) => b.releasedAt.localeCompare(a.releasedAt)).slice(0, 4);
    ...
    <Layout>
      <section className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Turn sparks into released work with steady momentum.</p>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {recentBlazes.map((b) => (
              <article key={b.id} className="rounded-xl border border-neon/20 bg-panelAlt p-4">
                ...
                <p className="text-xs text-muted">output type: release • {branchName(branches, b.branchId)} • {b.releasedAt}</p>
                <p className="mt-1 text-xs text-neonDim">value tags: consistency, visibility, signal</p>
              </article>
            ))}
          </div>
        </Section>
