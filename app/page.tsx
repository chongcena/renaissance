...
const Item=({title,text}:{title:string;text:string})=><p className='os-selectable rounded-lg border border-neon/10 bg-bg/30 p-2 text-sm'><span className='font-medium'>{title}</span><br/>{text}</p>;
function Stat({title,value}:{title:string;value:string}){return <article className='os-selectable rounded-xl border border-neon/10 bg-panelAlt/70 p-3'><p className='text-[11px] uppercase tracking-widest text-neonDim'>{title}</p><p className='mt-1 text-xl font-semibold text-amber-100'>{value}</p></article>}
function Panel({title,children}:{title:string;children:React.ReactNode}){return <section className='os-selectable rounded-xl border border-neon/15 bg-panelAlt/75 p-4'><h3 className='text-xs uppercase tracking-[0.2em] text-neonDim'>{title}</h3><div className='mt-3 space-y-2'>{children}</div></section>}
