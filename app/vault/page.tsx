...
export default function VaultPage() {
  ...
  const filteredSparks = useMemo(() => sparks.filter((s) => (stageFilter === 'all' || s.stage === stageFilter) && (statusFilter === 'all' || s.status === statusFilter)), [sparks, stageFilter, statusFilter]);

  return <Layout><h2 className="mb-4 text-xl font-semibold">Vault</h2><div className="mb-4 grid gap-2 sm:grid-cols-2"><select value={stageFilter} onChange={(e) => setStageFilter(e.target.value)} className="rounded-lg border border-neon/40 bg-panelAlt/85 px-3 py-2"><option value="all">All stages</option><option value="Spark">Spark</option><option value="Ember">Ember</option><option value="Fire">Fire</option><option value="Blaze">Blaze</option></select><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-lg border border-neon/40 bg-panelAlt/85 px-3 py-2"><option value="all">All statuses</option><option value="new">new</option><option value="active">active</option><option value="cooling">cooling</option><option value="frozen">frozen</option><option value="archived">archived</option><option value="killed">killed</option></select></div>
