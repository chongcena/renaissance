...
export default function SparkDetailPage() {
  ...
  <section className="mt-4 rounded-xl border border-neon/40 bg-panelAlt/85 backdrop-blur-sm p-4"><h4 className="text-sm uppercase tracking-wider text-neonDim">Pathways</h4><div className="mt-2 flex gap-2"><input value={lane} onChange={(e)=>setLane(e.target.value)} placeholder="New pathway lane" className="flex-1 rounded bg-bg p-2"/><button className="rounded bg-neon px-3 text-bg" onClick={()=>{if(lane){addPathway(spark.id,lane);setLane('');}}}>Add</button></div><ul className="mt-3 space-y-2 text-sm">{routes.map((r)=><li key={r.id} className="rounded border border-neon/40 p-2"><div className="flex justify-between"><span>{r.lane}</span><span>{r.confidence}%</span></div><div className="mt-2 flex gap-2"><button className="rounded bg-bg px-2 py-1" onClick={()=>updatePathway(r.id,{status:'frozen'})}>Freeze</button><button className="rounded bg-bg px-2 py-1" onClick={()=>updatePathway(r.id,{confidence:Math.min(100,r.confidence+5)})}>+Confidence</button></div></li>)}</ul></section>
  <section className="mt-4 rounded-xl border border-neon/40 bg-panelAlt/85 backdrop-blur-sm p-4"><h4 className="text-sm uppercase tracking-wider text-neonDim">Blaze Logs</h4><ul className="mt-3 space-y-2 text-sm">{sparkBlazes.length ? sparkBlazes.map((b) => <li key={b.id} className="rounded border border-neon/40 p-2">{b.title} • {b.releasedAt}</li>) : <li className="text-muted">No Blaze Logs yet.</li>}</ul></section>
