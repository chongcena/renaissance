...
export default function SparkDetailPage() {
  ...
  const sparkActions = actions.filter((a) => a.action.includes(id));
    ...
    return (
      <Layout>
        <section className="rounded-xl border border-neon/20 bg-panelAlt p-6">
          ...
          <Link href="/vault" className="mt-4 inline-block rounded bg-neon px-4 py-2 font-semibold text-black">Back to Vault</Link>
        </section>
      </Layout>
    );
  }
  ...
  <section className="mt-4 rounded-xl border border-neon/20 bg-panelAlt p-4"><h4 className="text-sm uppercase tracking-wider text-neonDim">Pathways</h4><div className="mt-2 flex gap-2"><input value={lane} onChange={(e)=>setLane(e.target.value)} placeholder="New pathway lane" className="flex-1 rounded bg-bg p-2"/><button className="rounded bg-neon px-3 text-black" onClick={()=>{if(lane){addPathway(spark.id,lane);setLane('');}}}>Add</button></div><ul className="mt-3 space-y-2 text-sm">{routes.map((r)=><li key={r.id} className="rounded border border-neon/20 p-2"><div className="flex justify-between"><span>{r.lane}</span><span>{r.confidence}%</span></div><div className="mt-2 flex gap-2"><button className="rounded bg-bg px-2 py-1" onClick={()=>updatePathway(r.id,{status:'frozen'})}>Freeze</button><button className="rounded bg-bg px-2 py-1" onClick={()=>updatePathway(r.id,{confidence:Math.min(100,r.confidence+5)})}>+Confidence</button></div></li>)}</ul></section>
  <section className="mt-4 rounded-xl border border-neon/20 bg-panelAlt p-4"><h4 className="text-sm uppercase tracking-wider text-neonDim">Blaze Logs</h4><ul className="mt-3 space-y-2 text-sm">{sparkBlazes.length ? sparkBlazes.map((b) => <li key={b.id} className="rounded border border-neon/20 p-2">{b.title} • {b.releasedAt}</li>) : <li className="text-muted">No Blaze Logs yet.</li>}</ul></section>
  <section className="mt-4 rounded-xl border border-neon/20 bg-panelAlt p-4"><h4 className="text-sm uppercase tracking-wider text-neonDim">Action Log History</h4><ul className="mt-3 space-y-2 text-sm">{sparkActions.length ? sparkActions.map((a) => <li key={a.id} className="rounded border border-neon/20 p-2">{a.date} • {a.action_type} • {a.action}</li>) : <li className="text-muted">No Spark-specific actions logged yet.</li>}</ul></section></Layout>;
}
