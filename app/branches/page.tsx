...
export default function BranchesPage() {
  ...
  return <Layout><h2 className="mb-4 text-xl font-semibold">Branches</h2>
    <form className="mb-4 grid gap-2 rounded-xl border border-neon/40 bg-panelAlt/85 p-4 sm:grid-cols-2" onSubmit={(e)=>{e.preventDefault();if(!createBranch({name,focus,heatScore,tags:tags.split(',').map(x=>x.trim())})) return; setName('');setFocus('');setTags('');setHeatScore(60);}}>
      ...
      <input type="number" min={0} max={100} value={heatScore} onChange={(e)=>setHeatScore(Number(e.target.value))} className="rounded bg-bg px-3 py-2" placeholder="Branch Weight" />
    ...
    <div className="grid gap-3 sm:grid-cols-2">{branches.map((b) => <article key={b.id} className="rounded-xl border border-neon/40 bg-panelAlt/85 p-4"><h3 className="text-lg font-semibold">{b.name}</h3><p className="text-sm text-muted">{b.focus || 'No focus yet.'}</p><p className="mt-2 text-sm text-fire">Branch Weight: {b.heatScore}</p><input value={b.focus} onChange={(e)=>updateBranch(b.id,{focus:e.target.value})} className="mt-2 w-full rounded bg-bg px-2 py-1 text-sm"/><div className="mt-2 flex gap-2"><button onClick={()=>updateBranch(b.id,{frozen:!b.frozen})} className="rounded border border-neon/40 px-2 py-1 text-xs">{b.frozen?'Unfreeze':'Freeze'}</button><label className="text-xs text-muted">Current Weight <input type="number" min={0} max={100} value={b.heatScore} onChange={(e)=>updateBranch(b.id,{heatScore:Number(e.target.value)})} className="ml-2 w-16 rounded bg-bg px-1 py-1"/></label></div></article>)}</div></Layout>;
}
