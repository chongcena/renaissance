...
export default function PillarsPage() {
  ...
  return <Layout><h2 className="mb-4 text-xl font-semibold">Creative Pillars</h2>
    <form className="mb-4 grid gap-2 rounded-xl border border-neon/40 bg-panelAlt/85 p-4 sm:grid-cols-2" onSubmit={(e)=>{e.preventDefault();if(!createBranch({name,focus,strategicWeight,role,color})) return; setName('');setFocus('');setStrategicWeight(20);setRole('Support');setColor('gold');}}>
    ...
    <div className="grid gap-3 sm:grid-cols-2">{branches.map((branch) => {const st=getPillarColorStyles(getPillarColor(branch)); const attention = branchAttention.find((item) => item.id === branch.id); const isEditing = editing === branch.id;
      return <article key={branch.id} className={`relative overflow-hidden rounded-xl border border-neon/35 bg-panelAlt/85 p-4 pl-5 before:absolute before:inset-y-0 before:left-0 before:w-1 ${st.leftRail} ${st.softBg} ${branch.frozen ? getFrozenStateClasses(true) : st.glow}`}><h3 className="flex items-center gap-2 text-lg font-semibold"><span className={`h-2.5 w-2.5 rounded-full ${st.dot}`} />{isEditing ? <input value={(draft.name ?? branch.name)} onChange={(e)=>setDraft((d)=>({ ...d, name: e.target.value }))} className="w-full rounded bg-bg px-2 py-1" /> : branch.name}</h3>
      ...
      <div className='mt-2 grid grid-cols-2 gap-2 text-xs'><p className='rounded border border-neon/20 px-2 py-1'>Strategic Weight <span className='ml-1 text-amber-100'>{branch.strategicWeight}%</span></p><p className='rounded border border-neon/20 px-2 py-1'>Actual Attention <span className='ml-1 text-amber-100'>{attention?.actual ?? 0}%</span></p></div><p className='mt-2 text-xs'><span className={`rounded-full border px-2 py-0.5 ${branch.frozen ? 'border-cyan-300/45 bg-cyan-300/16 text-cyan-100' : 'border-emerald-300/45 bg-emerald-300/16 text-emerald-100'}`}>{branch.frozen ? 'Frozen' : 'Active'}</span></p>
