...
import { PILLAR_COLORS, getFrozenStateClasses, getPillarColor, getPillarColorStyles, sectionStyles, motionStyles } from '@/lib/ui';
...
export default function PillarsPage() {
  ...
  return <Layout><h2 className="mb-4 text-xl font-semibold">Creative Pillars</h2>
    <form className={`mb-4 grid gap-2 sm:grid-cols-2 ${sectionStyles.active}`} onSubmit={(e)=>{e.preventDefault();if(!createBranch({name,focus,strategicWeight,role,color})) return; setName('');setFocus('');setStrategicWeight(20);setRole('Support');setColor('gold');}}>
      ...
      <button className={`rounded bg-neon px-4 py-2 font-semibold text-bg ${motionStyles.selectable}`}>Add Pillar</button>
    </form>
    <div className="grid gap-3 sm:grid-cols-2">{branches.map((branch) => {const st=getPillarColorStyles(getPillarColor(branch)); const attention = branchAttention.find((item) => item.id === branch.id); const isEditing = editing === branch.id;
      return <article key={branch.id} className={`os-selectable relative overflow-hidden rounded-xl border p-4 pl-5 ${branch.frozen ? getFrozenStateClasses(true) : `${st.border} ${st.card} ${st.glow}`}`}><span className={`absolute inset-y-0 left-0 w-1.5 ${branch.frozen ? 'bg-cyan-300' : st.rail}`} /><div className={`pointer-events-none absolute inset-x-0 top-0 h-8 bg-gradient-to-r ${branch.frozen ? 'from-cyan-300/25 to-transparent' : st.topAccent}`} /><h3 className="relative flex items-center gap-2 text-lg font-semibold"><span className={`inline-block h-2.5 w-2.5 rounded-full ${branch.frozen ? 'bg-cyan-200' : st.dot}`} />{isEditing ? <input value={(draft.name ?? branch.name)} onChange={(e)=>setDraft((d)=>({ ...d, name: e.target.value }))} className="w-full rounded bg-bg px-2 py-1" /> : branch.name}</h3>
      ...
      <div className="mt-3 flex gap-2">{isEditing ? <><button onClick={()=>{updateBranch(branch.id, draft); setEditing(null); setDraft({});}} className="rounded bg-neon px-3 py-1 text-xs font-semibold text-bg">Save</button><button onClick={()=>{setEditing(null); setDraft({});}} className={`rounded border border-neon/40 px-3 py-1 text-xs ${motionStyles.selectable}`}>Cancel</button></> : <button onClick={()=>{setEditing(branch.id); setDraft(branch);}} className={`rounded border border-neon/40 px-3 py-1 text-xs ${motionStyles.selectable}`}>Edit Pillar</button>}<button onClick={()=>updateBranch(branch.id,{frozen:!branch.frozen})} className={`rounded border border-neon/40 px-3 py-1 text-xs ${motionStyles.selectable}`}>{branch.frozen ? 'Set Active' : 'Freeze Pillar'}</button><Link href={`/goals?pillarId=${branch.id}`} className={`rounded border border-neon/40 px-2 py-1 text-xs ${motionStyles.selectable}`}>Add Goal</Link></div></article>;})}</div></Layout>;
}
