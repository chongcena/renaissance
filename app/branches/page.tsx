...
import { PILLAR_COLORS, getFrozenStateClasses, getPillarColor, getPillarColorStyles, motionStyles } from '@/lib/ui';
...
export default function PillarsPage() {
  ...
  return <Layout><h2 className="mb-4 text-xl font-semibold">Creative Pillars</h2>
    ...
    <div className="grid gap-3 sm:grid-cols-2">{branches.map((branch) => {const st=getPillarColorStyles(getPillarColor(branch)); const attention = branchAttention.find((item) => item.id === branch.id); const isEditing = editing === branch.id;
      ...
      <div className="mt-3 flex gap-2">{isEditing ? <><button onClick={()=>{updateBranch(branch.id, draft); setEditing(null); setDraft({});}} className="rounded bg-neon px-3 py-1 text-xs font-semibold text-bg">Save</button><button onClick={()=>{setEditing(null); setDraft({});}} className="os-selectable rounded border border-neon/40 px-3 py-1 text-xs">Cancel</button></> : <button onClick={()=>{setEditing(branch.id); setDraft(branch);}} className="os-selectable rounded border border-neon/40 px-3 py-1 text-xs">Edit Pillar</button>}<button onClick={()=>updateBranch(branch.id,{frozen:!branch.frozen})} className="os-selectable rounded border border-neon/40 px-3 py-1 text-xs">{branch.frozen ? 'Set Active' : 'Freeze Pillar'}</button><button onClick={()=>createGoal({title:`${branch.name} Goal`, pillarId:branch.id, status:'active', scale:'month'})} className='os-selectable rounded border border-neon/40 px-2 py-1 text-xs'>Add Goal</button></div></article>;})}</div></Layout>;
}
