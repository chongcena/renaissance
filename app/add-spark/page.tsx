...
export default function AddSparkPage() {
  ...
  return <Layout><section className="space-y-4"><h2 className="text-xl font-semibold">Add Spark</h2><form className="space-y-3 rounded-xl border border-neon/40 bg-panelAlt/85 backdrop-blur-sm p-4" onSubmit={(e)=>{e.preventDefault();const createdId = createSpark({title,kind,branchId,notes}); if (!createdId) { setTitleError('Spark title is required.'); return; } setTitleError(''); setTitle('');setNotes(''); router.push(`/spark/${createdId}`);}}>
    ...
    <button className="rounded-lg bg-neon px-4 py-2 font-semibold text-bg">Create Spark Item</button></form></section></Layout>;
}
