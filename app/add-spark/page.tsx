...
export default function AddSparkPage() {
  ...
  return <Layout><section className="space-y-4"><h2 className="text-xl font-semibold">Add Spark</h2><form className="space-y-3 rounded-xl border border-neon/40 bg-panelAlt/85 p-4 backdrop-blur-sm" onSubmit={(e)=>{e.preventDefault();if (!title.trim()) { setTitleError('Spark title is required.'); return; }const createdId = createSpark({title,kind,branchId,notes}); if (!createdId) { setTitleError('Spark title is required.'); return; } setTitleError(''); router.push(`/spark/${createdId}`);}}>
