          ...
          {nav.map((item) => {
            ...
            return (
              <Link
                key={item.id}
                href={item.href}
                className={`os-selectable rounded-lg px-3 py-1.5 transition ${
                  active ? 'bg-neon/20 text-amber-100 shadow-[0_0_16px_rgba(251,191,36,0.2)]' : 'text-muted hover:bg-panelAlt/70 hover:text-text'
                }`}
              >
                {item.label}
              </Link>
