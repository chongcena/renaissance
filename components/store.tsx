...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<Store>(() => ({
    ...
    updatePathway: (pid, patch) => { const path=paths.find((p)=>p.id===pid); setPaths((p)=>p.map((i)=>i.id===pid?{...i,...patch,last_touched_at:today()}:i)); if(patch.currentAction!==undefined) appendAction('set_next_move',`Set Current Action: ${patch.currentAction || 'Cleared Current Action'}`); appendAction('progress',`Updated Pathway: ${path?.title ?? pid}`); },
